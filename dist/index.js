...
var copyFile = async (file, rootSrc, rootDest, targets, structured) => {
  const copyTargets = await collectCopyTargets(rootSrc, targets, structured);
  const changedFiles = [];
  for (const copyTarget of copyTargets) {
    const { src, dest, transform, preserveTimestamps, dereference, overwrite } = copyTarget;
...
var removeFile = async (file, rootSrc, rootDest, targets, structured) => {
  const copyTargets = await collectCopyTargets(rootSrc, targets, structured);
  const changedFiles = [];
  for (const copyTarget of copyTargets) {
...
var servePlugin = ({
  ...
  return {
    ...
    configureServer({ httpServer, middlewares, ws }) {
      ...
      if (watch.copyToDest) {
        copyAll(config.root, config.build.outDir, targets, structured).then(
          (copyCount) => {
